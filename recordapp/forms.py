...
from . models import Patient_info, Address, Next_of_kin, Personnel_record
...
class Patient_info_form(forms.ModelForm):
    ...
    sex = forms.ChoiceField(choices=CHOICES, widget=forms.RadioSelect( attrs={'class':'form-check-inline', 'type':'radio', 'name':'gender' ,'id':'gender', 'style': 'display: inline-block'}))
    PATIENT_STATUS = [
        ('DHML', 'DHML'),
        ('NHIS', 'NHIS'),
        ('Not NHIS', 'Not NHIS'),
        ]
    status = forms.ChoiceField(choices=PATIENT_STATUS,  widget=forms.RadioSelect(attrs={'class':'form-check-inline hide', 'type':'radio', 'name':'status' ,'id':'status'}))

    PATIENT_TYPE = [
        ('CIV', 'Civillian'),
        ('Pesonnel', 'Personnel'),
        ]
    patient_type = forms.ChoiceField(choices=PATIENT_TYPE, widget=forms.RadioSelect( attrs={'class':'form-check-inline', 'type':'radio', 'name':'patientType' ,'id':'patientType'}))
